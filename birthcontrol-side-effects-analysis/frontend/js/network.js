...
function handleSearch(event) {
    ...
    const matchingNodes = allNodes.filter(node =>
        node.id.toLowerCase().includes(filters.search)
    );
    ...
    d3.selectAll('.link')
        .classed('faded', d => !matchingIds.has(d.source.id) && !matchingIds.has(d.target.id));
...
function applyFilters() {
    ...
    const filteredNodes = allNodes.filter(node => {
        ...
        // Apply type filter
        if (filters.type === 'mental' && !isMentalSymptom(node.id)) return false;
        if (filters.type === 'physical' && !isPhysicalSymptom(node.id)) return false;
...
function updateVisibleStats(visibleNodes = null, visibleEdges = null) {
    ...
    if (visibleNodes === null) {
        ...
        visibleNodes = d3.selectAll('.node').filter(function() {
            return d3.select(this).style('display') !== 'none' && !d3.select(this).classed('faded');
        }).size();

        visibleEdges = d3.selectAll('.link').filter(function() {
            return d3.select(this).style('display') !== 'none' && !d3.select(this).classed('faded');
        }).size();
    }
...
function isMentalSymptom(symptomId) {
    const mentalSymptoms = ['anxiety', 'depression', 'mood', 'panic', 'brain', 'fog',
                           'nervousness', 'fear', 'sadness', 'crying', 'emotional',
                           'irritability', 'anger', 'suicidal'];
    return mentalSymptoms.some(mental => symptomId.toLowerCase().includes(mental));
}

function isPhysicalSymptom(symptomId) {
    const physicalSymptoms = ['acne', 'weight', 'hair', 'cramps', 'bleeding', 'spotting',
                             'headache', 'nausea', 'fatigue', 'sensitivity', 'libido'];
...
function showDetailsPanel(node, connectedNodes, connectedLinks) {
    ...
    if (connectedLinks.size > 0) {
        ...
        const sortedLinks = Array.from(connectedLinks)

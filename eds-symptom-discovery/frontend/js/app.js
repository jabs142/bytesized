...
function renderAllSymptomsTable() {
    ...
    let symptomsToShow = allSymptoms.filter(symptom => {
        const isValidated = validatedNames.has(symptom.symptom);
        if (isValidated && showValidated) return true;
        if (!isValidated && showUnvalidated) return true;
        return false;
    });
    ...
    paginatedSymptoms.forEach((symptom, index) => {
        ...
        if (isValidated) {
            ...
            if (surpriseScore > 0.1) badgeClass = 'badge-warning';
            if (surpriseScore > 0.3) badgeClass = 'badge-info';

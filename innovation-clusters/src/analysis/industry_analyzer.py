...
Analyzes each industry category separately and compares trends
"""
import pandas as pd
import numpy as np
import json
...
class IndustryAnalyzer:
    """Analyzes innovation trends by industry"""
    ...
    def analyze(self) -> Dict:
        ...
        if not patent_df.empty:
            for category in patent_df['technology_category'].unique():
                ...
                industries[cat_id] = self._analyze_category(
                    cat_df,
                    category,
                    None  # Will extract from clusters if needed
                )

        # Therapeutic area analysis (for drugs)
        therapeutic_areas = {}
        if not fda_df.empty and 'submission_type' in fda_df.columns:
            therapeutic_areas = self._analyze_therapeutic_areas(fda_df)

        # Decade breakdown
        decade_breakdown = self._analyze_by_decade(fda_df, patent_df)
    ...
    def _analyze_by_decade(
        self,
        fda_df: pd.DataFrame,
        patent_df: pd.DataFrame
    ) -> Dict:
        """Analyze innovations by decade"""
        ...
        if not patent_df.empty:
            for _, row in patent_df.iterrows():
                if pd.notna(row['year']):
                    ...
                    category = row['technology_category'].lower()
                    if category in decade_data[decade]:
                        decade_data[decade][category] += 1

"""
Pharmaceutical Visualization Data Generator

Exports JSON data for FDA drug approval charts and visualizations
"""
import pandas as pd
import numpy as np
import json
...
class VizGenerator:
    """Generates pharmaceutical visualization-ready JSON data"""
    ...
    def generate_all(self):
        """Generate all pharmaceutical visualization data files"""

        print("\n" + "="*70)
        print("GENERATING PHARMACEUTICAL VISUALIZATION DATA")
        print("="*70)
        ...
        timeline_data = self._generate_timeline(fda_df, stats)
    ...
    def _generate_timeline(self, fda_df: pd.DataFrame, stats: Dict) -> Dict:
        """Generate FDA drug approval timeline data"""

        print("\n📊 Generating FDA drug approval timeline...")
        ...
        # Count FDA drug approvals per year
        fda_counts = fda_df['year'].value_counts().to_dict()
        series = {
            'fda_drugs': [int(fda_counts.get(year, 0)) for year in years]
        }

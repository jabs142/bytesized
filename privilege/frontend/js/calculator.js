...
class LifeStatsCalculator {
    constructor() {
        ...
        this.WEIGHTS = {
            ...
            education: 0.20,     // HDI component, enables opportunity
            healthcare: 0.20,    // Life-or-death, health outcomes
            water: 0.15,         // Basic survival need
            location: 0.10,      // Infrastructure, safety, rule of law
    ...
    calculateLifeStatsScore(answers) {
        ...
        if (answers.income?.option?.value) {
            ...
            percentiles.income = applyUncertainty(
                incomePercentiles[answers.income.option.value] || 45
            );
            factorDetails.push({
                factor: 'income',
        ...
        if (answers.water?.option?.value) {
            ...
            percentiles.water = applyUncertainty(
                waterPercentiles[answers.water.option.value] || 50
            );
            factorDetails.push({
                factor: 'water',
        ...
        if (answers.hunger?.option?.value) {
            ...
            percentiles.hunger = applyUncertainty(
                hungerPercentiles[answers.hunger.option.value] || 50
            );
            factorDetails.push({
                factor: 'hunger',

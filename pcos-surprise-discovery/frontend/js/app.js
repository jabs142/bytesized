...
function filterSymptoms(symptoms, filter) {
  ...
  return symptoms.filter((symptom) => {
    ...
    if (filter === 'very') {
      return classification.includes('VERY SURPRISING');
    }
    if (filter === 'somewhat') {
      return classification.includes('SOMEWHAT SURPRISING');
    }
    if (filter === 'expected') {
...
function createSymptomCard(symptom, rank) {
  ...
  if (evidence.reddit_posts) {
    evidenceItems.push(
      `${evidence.reddit_posts} patient reports (${(evidence.reddit_frequency * 100).toFixed(1)}%)`
    );
  }
  ...
  if (factors.not_in_criteria) {
    surpriseFactorsList.push('Not in diagnostic criteria');
  }
  if (factors.low_public_awareness) {
    surpriseFactorsList.push('Low public awareness');
  }
  if (factors.high_surprise_signals) {
    surpriseFactorsList.push('Patients frequently surprised');
  }
  if (factors.research_validated) {
    surpriseFactorsList.push('Research validated');
  }
